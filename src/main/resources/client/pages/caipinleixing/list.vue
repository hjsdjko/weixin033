...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
			<view :style='{"minHeight":"100rpx","width":"100%","position":"relative","alignItems":"center","justifyContent":"space-between","display":"flex"}'>
				<view :style='{"backgroundColor":"#f5f5f5","margin":"0 30rpx 0 30rpx","color":"#333333","alignItems":"center","borderRadius":"40rpx","flex":"1","display":"flex","lineHeight":"64rpx","fontSize":"24rpx","height":"64rpx"}'>
					...
					<input v-model="searchForm.caipinleixing" type="text" placeholder="菜品类型" :style='{"background":"transparent","height":"100%"}'></input>
				</view>
				...
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 0px 0px #00000020","margin":"0 0 40rpx 0","borderColor":"#06988850","flexWrap":"wrap","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"dashed","justifyContent":"center","height":"auto"}' v-for="(product,index) in list" :key="index">
				  ...
				  <view class="listmr" :style='{"width":"100%","padding":"0 0 0 0","margin":"0 0 0 0","height":"auto"}'>
					...
					<view :style='{"width":"100%","padding":"40rpx 20rpx 40rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('caipinleixing','修改')) || (!userid && isAuthFront('caipinleixing','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('caipinleixing','删除')) || (!userid && isAuthFront('caipinleixing','删除'))" @click.stop="onDeleteTap(product.id)">
		...
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #cccccc","color":"#333333","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#75ebde","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('caipinleixing','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"0 2rpx 12rpx #cccccc","color":"#333333","bottom":"120rpx","right":"120rpx","outline":"none","borderRadius":"100%","background":"#75ebde","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"fixed","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('caipinleixing','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.caipinleixing){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`caipinleixing`, params);
                } else {
                    res = await this.$api.list(`caipinleixing`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('caipinleixing', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.caipinleixing){
					searchForm['caipinleixing'] = '%' + this.searchForm.caipinleixing + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`caipinleixing`, searchForm);
                } else {
                    res = await this.$api.list(`caipinleixing`, searchForm);
                }
				// 如果是第一页数据置空
